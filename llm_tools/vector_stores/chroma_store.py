...
class ChromaStore(VectorStore):
    ...
    def search_by_vector(self, vector: np.ndarray, n_results: int = 5) -> list[RetrievedDocument]:
        results = self.collection.query(
            query_embeddings=[vector.tolist()], n_results=n_results, include= ["distances", "documents", "metadatas"]
        )
        ...
        for doc_id, text, metadata, distance in zip(
            ...
            documents.append(RetrievedDocument(id=doc_id, text=text, similarity_score=distance, metadata=metadata or {}))
